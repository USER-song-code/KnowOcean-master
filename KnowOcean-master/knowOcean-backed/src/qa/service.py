...
async def plan_query(question: str) -> list[str]:
    """LLM 查询规划 → 最多 3 条优化查询"""
    try:
        resp = await llm.chat([
...
def keyword_search_java(query: str, group_id: int, top_k: int = 50) -> list[dict]:
    ...
    s = get_settings()
    try:
        es = Elasticsearch(f"http://{s.es_host}:{s.es_port}")
        body = {
            "size": top_k,
...
async def hybrid_retrieve(group_id: int, question: str) -> list[dict]:
    ...
    """
    # 1. 查询规划
    planned_queries = await plan_query(question)
...
async def ask_question(group_id: int, question: str) -> dict:
    ...
    # 1. 混合检索 + 聚类 + 证据评估
    evidence_docs = await hybrid_retrieve(group_id, question)
...
async def stream_ask(group_id: int, question: str):
