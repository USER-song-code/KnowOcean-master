...
<template>
  <div class="doc-stats">
    ...
    <div class="doc-stats__header">
      <h3 class="doc-stats__title">文档统计</h3>
      <button
        class="doc-stats__refresh-btn"
        :class="{ 'doc-stats__refresh-btn--spinning': refreshing }"
        :disabled="refreshing"
        @click="emit('refresh')"
        title="刷新数据"
      >
...
.doc-stats__refresh-btn:hover:not(:disabled) {
  background: rgba(74, 144, 217, 0.15);
  border-color: rgba(74, 144, 217, 0.35);
  transform: translateY(-1px);
}

.doc-stats__refresh-btn:active:not(:disabled) {
  transform: translateY(0);
}

.doc-stats__refresh-btn:disabled {
  opacity: 0.6;
  cursor: not-allowed;
}

.doc-stats__refresh-btn svg {
  width: 16px;
  height: 16px;
  transition: transform 0.6s cubic-bezier(0.4, 0, 0.2, 1);
}

.doc-stats__refresh-btn--spinning svg {
