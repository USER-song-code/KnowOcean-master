...
export async function fetchAssistantSessions(): Promise<AssistantSessionListItem[]> {
  const { data } = await http.get<ApiResponse<AssistantSessionListItem[]>>('/assistant/sessions')
...
export async function fetchAssistantSessionDetail(sessionId: number): Promise<AssistantSessionDetail> {
  const { data } = await http.get<ApiResponse<AssistantSessionDetail>>(`/assistant/sessions/${sessionId}`)
  if (!data.success || data.data == null) {
...
export async function fetchAssistantConversationContext(
  ...
  const { data } = await http.get<ApiResponse<AssistantConversationContext>>(`/assistant/sessions/${sessionId}/context`, {
    params: { recentLimit },
  })
  if (!data.success || data.data == null) {
    throw new Error(data.message ?? '获取会话上下文失败')
  }
  return data.data
}
