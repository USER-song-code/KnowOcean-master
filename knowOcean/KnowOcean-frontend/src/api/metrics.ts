...
/**
 * 获取用户使用详情（模型拆分、模块拆分、趋势、文档数、提问数）
 *
 * GET /api/admin/metrics/user/{userId}/detail?period=
 */
export async function fetchUserUsageDetail(userId: number, period: Period): Promise<UserUsageDetail> {
  const { data } = await http.get<ApiResponse<UserUsageDetail>>(`/admin/metrics/user/${userId}/detail`, {
    params: { period },
  })
  return unwrapApiResponse(data, '加载用户使用详情失败')
}
