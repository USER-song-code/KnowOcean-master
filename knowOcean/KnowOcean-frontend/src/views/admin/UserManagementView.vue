...
// ── 使用统计（详情面板内） ──
const userUsageDetail = ref<UserUsageDetail | null>(null)
const isUsageLoading = ref(false)
const usagePeriod = ref<Period>('LAST_7_DAYS')

const periodOptions: { key: Period; label: string }[] = [
  { key: 'TODAY', label: '今日' },
  { key: 'LAST_7_DAYS', label: '7天' },
  { key: 'LAST_14_DAYS', label: '14天' },
  { key: 'LAST_30_DAYS', label: '30天' },
]
...
function getModuleLabel(module: string): string {
  return module === 'QA' ? '知识问答' : module === 'ASSISTANT' ? 'AI 助手' : module
}
...
async function openUserDetail(userId: number) {
  ...
  try {
    ...
    try {
      userUsageDetail.value = await fetchUserUsageDetail(userId, usagePeriod.value)
    } catch {
      // 使用数据加载失败不阻塞详情面板
      userUsageDetail.value = null
    } finally {
...
watch(usagePeriod, () => {
  if (detailUser.value) {
    void loadUsageData(detailUser.value.userId)
  }
})
</script>
            ...
            <div class="detail-panel__body">
              ...
              <div v-else-if="detailUser" class="detail-content">
                ...
                <div class="usage-section">
                  <h4 class="usage-section__title">使用统计</h4>
                  ...
                  <div class="period-tabs">
                    <button
                      ...
                      :key="p.key"
                      class="period-tab"
                      :class="{ 'is-active': usagePeriod === p.key }"
                      type="button"
                      @click="usagePeriod = p.key"
                    >{{ p.label }}</button>
                  </div>

                  <!-- 加载中 -->
                  <div v-if="isUsageLoading" class="usage-loading">
                    <div class="spinner spinner--sm"></div>
                    <span>加载使用数据...</span>
                  </div>

                  <!-- 无数据 -->
                  <div v-else-if="!userUsageDetail" class="usage-empty">
                    <span>暂无使用数据</span>
                  </div>
                  ...
                  <template v-else>
                    ...
                    <div class="usage-kpi-grid">
                      <div class="usage-kpi">
                        <span class="usage-kpi__label">请求数</span>
                        <span class="usage-kpi__value">{{ formatNumber(userUsageDetail.usageStats.totalRequests) }}</span>
                      </div>
                      <div class="usage-kpi">
                        <span class="usage-kpi__label">Token</span>
                        <span class="usage-kpi__value">{{ formatNumber(userUsageDetail.usageStats.totalTokens) }}</span>
                      </div>
                      <div class="usage-kpi">
                        <span class="usage-kpi__label">费用</span>
                        <span class="usage-kpi__value">{{ formatCost(userUsageDetail.usageStats.totalCost) }}</span>
                      </div>
                      <div class="usage-kpi">
                        <span class="usage-kpi__label">延迟</span>
                        <span class="usage-kpi__value">{{ formatLatency(userUsageDetail.usageStats.avgLatencyMs) }}</span>
                      </div>
                    </div>

                    <!-- 模型用量表 -->
                    <div v-if="userUsageDetail.modelBreakdown.length > 0" class="usage-table-wrap">
                      <h5 class="usage-subtitle">模型用量</h5>
                      <table class="usage-mini-table">
                        <thead>
                          <tr>
                            <th>模型</th>
                            <th class="num">请求</th>
                        ...
                        <tbody>
                          <tr v-for="m in userUsageDetail.modelBreakdown" :key="m.modelName">
                            <td class="model-name">{{ m.modelName }}</td>
                            <td class="num">{{ m.requests }}</td>
                            <td class="num">{{ formatNumber(m.totalTokens) }}</td>
                            <td class="num cost-col">{{ formatCost(m.cost) }}</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                    ...
                    <!-- 模块拆分 -->
                    <div v-if="userUsageDetail.moduleBreakdown.length > 0" class="usage-table-wrap">
                      <h5 class="usage-subtitle">功能模块</h5>
                      <table class="usage-mini-table">
                        <thead>
                          <tr>
                            <th>模块</th>
                        ...
                        <tbody>
                          <tr v-for="m in userUsageDetail.moduleBreakdown" :key="m.module">
                            <td>{{ getModuleLabel(m.module) }}</td>
                            <td class="num">{{ m.requests }}</td>
                            <td class="num">{{ formatNumber(m.totalTokens) }}</td>
                            <td class="num cost-col">{{ formatCost(m.cost) }}</td>
                          </tr>
                    ...
                    <div class="usage-summary">
                      <div class="usage-summary__item">
                        <span class="usage-summary__label">上传文档</span>
                        <span class="usage-summary__value">{{ userUsageDetail.documentCount }}</span>
                      </div>
                      <div class="usage-summary__item">
                        <span class="usage-summary__label">提问次数</span>
                        <span class="usage-summary__value">{{ userUsageDetail.questionCount }}</span>
                      </div>
                      <div class="usage-summary__item">
                        <span class="usage-summary__label">成功率</span>
                        <span class="usage-summary__value">{{ userUsageDetail.usageStats.successRate }}%</span>
                      </div>
                    </div>
                  </template>
                </div>
...
/* ════════════ Usage Stats Section (Detail Panel) ════════════ */
.usage-section {
  margin-top: 6px;
  padding-top: 18px;
  border-top: 1px solid var(--border-subtle);
...
.usage-mini-table td.num {
  text-align: right;
  font-variant-numeric: tabular-nums;
}

.usage-mini-table td.cost-col {
  color: var(--text-primary);
  font-weight: 600;
}

.usage-mini-table .model-name {
  font-family: 'JetBrains Mono', 'Consolas', monospace;
  font-size: 0.72rem;
  color: var(--text-primary);
}

.usage-mini-table tbody tr:last-child td {
  border-bottom: none;
}
