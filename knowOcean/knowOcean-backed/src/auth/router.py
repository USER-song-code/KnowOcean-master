...
async def login(body: LoginRequest, response: Response, db: AsyncSession = Depends(get_db)):
    ...
    await record_operation(
        user_id=user.id, username=user.username, category="AUTH", action="LOGIN",
        detail={"loginMethod": "password"},
    )
    return ApiResponse.ok(data=_build_session(access_token, user))
...
async def logout(
    ...
    await record_operation(
        user_id=user.user_id, username=user.username, category="AUTH", action="LOGOUT",
    )
