...
from src.logging.operation_recorder import record_operation
...
async def upload_document(
    ...
    await record_operation(
        user_id=user.user_id, username=user.username, category="DOCUMENT", action="DOC_UPLOAD",
        target_type="document", target_id=doc.id,
        detail={"fileName": file.filename or "unnamed", "fileSize": len(content), "groupId": groupId},
    )
...
async def delete_document(document_id: int, user: UserIdentity = Depends(require_auth), db: AsyncSession = Depends(get_db)):
    ...
    await record_operation(
        user_id=user.user_id, username=user.username, category="DOCUMENT", action="DOC_DELETE",
        target_type="document", target_id=document_id,
        detail={"fileName": doc.file_name, "groupId": doc.group_id},
    )
