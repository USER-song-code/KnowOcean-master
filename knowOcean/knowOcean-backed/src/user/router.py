...
async def handle_update_user_status(
    ...
    action = "USER_ENABLE" if body.status == "ACTIVE" else "USER_DISABLE"
    await record_operation(
        user_id=admin.user_id, username=admin.username, category="ADMIN", action=action,
        target_type="user", target_id=user_id,
        detail={"targetUser": target_user.username if target_user else str(user_id), "newStatus": body.status},
    )
