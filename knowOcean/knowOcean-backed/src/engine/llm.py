...
def _get_client() -> AsyncOpenAI:
    ...
    if _client is None:
        ...
        if not base.endswith("/v1"):
            base += "/v1"
        # 显式禁用代理，避免新版 openai/httpx 报错
        http_client = httpx.AsyncClient(proxy=None, timeout=httpx.Timeout(300.0, connect=30.0))
        _client = AsyncOpenAI(api_key=settings.dashscope_api_key, base_url=base, http_client=http_client)
    return _client
