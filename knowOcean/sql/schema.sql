-- ===================================================
-- KnowOcean 数据库初始化 DDL（PostgreSQL）
-- ===================================================
...
CREATE TABLE IF NOT EXISTS operation_logs (
    ...
    user_id      BIGINT       NOT NULL,
    username     VARCHAR(64),
    category     VARCHAR(32)  NOT NULL,   -- AUTH | ADMIN | DOCUMENT
    action       VARCHAR(64)  NOT NULL,   -- LOGIN / LOGOUT / USER_ENABLE / USER_DISABLE / DOC_UPLOAD / DOC_DELETE
    target_type  VARCHAR(32),             -- 操作目标类型 (user, document 等)
    target_id    BIGINT,                  -- 操作目标 ID
    detail       JSONB,                   -- 额外详情 (文件名、文件大小、IP 等)
    ip_address   VARCHAR(64),
    created_at   TIMESTAMP    NOT NULL DEFAULT now()
);

COMMENT ON TABLE  operation_logs             IS '系统操作日志表（审计追踪）';

COMMENT ON COLUMN operation_logs.user_id     IS '操作用户 ID';
COMMENT ON COLUMN operation_logs.username    IS '操作用户名（冗余，方便查询）';
COMMENT ON COLUMN operation_logs.category    IS '操作类别: AUTH | ADMIN | DOCUMENT';
COMMENT ON COLUMN operation_logs.action      IS '操作: LOGIN | LOGOUT | USER_ENABLE | USER_DISABLE | DOC_UPLOAD | DOC_DELETE';
COMMENT ON COLUMN operation_logs.target_type IS '操作目标类型';
COMMENT ON COLUMN operation_logs.target_id   IS '操作目标 ID';
COMMENT ON COLUMN operation_logs.detail      IS '额外详情 (JSON)';
COMMENT ON COLUMN operation_logs.ip_address  IS '客户端 IP';
COMMENT ON COLUMN operation_logs.created_at  IS '操作时间';

CREATE INDEX IF NOT EXISTS idx_oplog_user_created
    ON operation_logs (user_id, created_at);
CREATE INDEX IF NOT EXISTS idx_oplog_category_created
    ON operation_logs (category, created_at);
